...
class SecurityReviewerTest {
    ...
    private SecurityReviewer reviewer;

    @BeforeEach
    void setUp() {
        reviewer = new SecurityReviewer(chatClient, standardsRetrieverService);
    }

    @Test
    void testType() {
        assertEquals(ReviewerType.SECURITY, reviewer.type());
    }
    ...
    void testReview_withEmptyHunks() {
        ...
        assertEquals("Security review (grounded) complete", result.summary());
    ...
    void testReview_withValidHunks() {
        ...
        String standardsContext = "Security standards content";
        when(standardsRetrieverService.retrieveContext(anyString(), anyInt(), anyString()))
    ...
    @Test
    void testReview_withNoSecurityIssues() {
        // Arrange
        List<DiffHunk> hunks = List.of(
                new DiffHunk("TestFile.java", 1, 10, "diff content")
        );
